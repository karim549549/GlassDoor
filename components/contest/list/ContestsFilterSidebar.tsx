...
interface ContestsFilterSidebarProps {
  searchQuery: string;
  ...
  accessFilter: ContestAccessFilter;
  onAccessChange: (access: ContestAccessFilter) => void;
  sortBy: ContestSortOption;
  onSortChange: (sort: ContestSortOption) => void;
}

/** Neo-brutalist search box, scope tabs, and filter/sort controls for the contest registry. */
export function ContestsFilterSidebar({
  searchQuery,
  onSearchChange,
  activeTab,
  onTabChange,
  ...
  return (
    <div className="border-2 border-[#0E0E0D] bg-white p-5 shadow-[4px_4px_0px_0px_#0E0E0D] space-y-4">
      ...
      <div className="flex flex-col gap-2 pt-2 border-t border-dashed border-[#0E0E0D]/10">
        ...
        <ContestTabs activeTab={activeTab} setActiveTab={onTabChange} allCount={allCount} myCount={myCount} />
      </div>
      ...
      <div className="flex flex-col gap-1 pt-2 border-t border-dashed border-[#0E0E0D]/10">
        ...
        <select
          value={statusFilter}
          onChange={(e) => onStatusChange(e.target.value as ContestStatusFilter)}
      ...
      <div className="flex flex-col gap-1 pt-2 border-t border-dashed border-[#0E0E0D]/10">
        ...
        <select
          value={accessFilter}
          onChange={(e) => onAccessChange(e.target.value as ContestAccessFilter)}
          className="w-full bg-[#FAF8F5] border-2 border-[#0E0E0D] px-2 py-1.5 font-mono text-[0.6rem] uppercase focus:outline-none focus:border-orange cursor-pointer"
        >
      ...
      <div className="flex flex-col gap-1 pt-2 border-t border-dashed border-[#0E0E0D]/10">
        ...
        <select
          value={sortBy}
          onChange={(e) => onSortChange(e.target.value as ContestSortOption)}
          className="w-full bg-[#FAF8F5] border-2 border-[#0E0E0D] px-2 py-1.5 font-mono text-[0.6rem] uppercase focus:outline-none focus:border-orange cursor-pointer"
        >
