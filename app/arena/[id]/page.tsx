...
export default async function ArenaDetailPage({ params }: PageProps) {
  ...
  try {
    ...
    // Prototype fallback if database item is missing or during development testing
    console.warn("Internal API fetch failed, loading prototype fallback state.");
  }

  // Prototype visual demonstration data if DB record is incomplete or for prototype browsing
  if (!arenaData) {
    arenaData = {
