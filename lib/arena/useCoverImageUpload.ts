...
/**
 * Owns the cover-image upload flow for the arena create form: reading the
 ...
 */
export function useCoverImageUpload(setValue: UseFormSetValue<ArenaFormInput>) {
  ...
  const handleCroppedUpload = async (blob: Blob) => {
    ...
    try {
      const res = await fetch("/api/arena/upload", {
        method: "POST",
        body: formData,
      });
